...
function ResultsPhase({ result, aiRecs, onReset, onBack }) {
  ...
  return createPortal(
    <div style={{
      ...
      {/* ── RIGHT PANEL: bare gradient + floating brain ── */}
...
export default function AIDiagnosis({ onBack, onPhaseChange }) {
  ...
  const onSubmit = async e => {
    ...
    try {
      ...
      // Fire API call — animation happens after response in setPhaseWithCallback
      ...
      {phase === 'input' && (
        <div ref={inputWrapRef}>
          <InputPhase
            ...
            onBack={onBack}
          />
          {/* Pre-warm the vivid BrainCanvas (results brain) while user fills the form.
              Mounted invisible so WebGL context + shader compile happen in background. */}
          <div style={{ position: 'fixed', width: 1, height: 1, opacity: 0, pointerEvents: 'none', overflow: 'hidden', top: 0, left: 0, zIndex: -1 }}>
            <BrainCanvas vivid />
          </div>
        </div>
      )}

      {/* RESULTS PHASE — portaled to document.body, true fullscreen.
          Brain canvas lives inside ResultsPhase portal so it's always
          at the correct z-level. Pre-warm it by mounting immediately
          when we have a result queued (invisible until phase flips). */}
