...
export default function AlzheimerCareApp() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setAuthError('Failed to connect to server. Make sure the backend is running on port 5000.');
    } finally {
  ...
  const handlePredictionSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5001/api/predictions/predict-enhanced', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      {!isAuthenticated ? (
        /* ===== Auth Screen ===== */
        <div className="flex items-center justify-center w-full min-h-screen bg-gradient-to-br from-purple-900 via-purple-700 to-pink-500">
          <div className="w-full max-w-md mx-4">
            {/* Logo */}
            <div className="text-center mb-8">
              <div className="inline-flex items-center justify-center bg-white p-4 rounded-2xl shadow-xl mb-4">
                <Brain className="h-10 w-10 text-purple-700" />
              </div>
              <h1 className="text-3xl font-bold text-white">Remindly</h1>
              <p className="text-purple-200 mt-1">Alzheimer's Care Dashboard</p>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-2xl p-8">
              ...
              <div className="flex mb-6 bg-gray-100 rounded-lg p-1">
                <button
                  ...
                  className={`flex-1 py-2.5 text-sm font-semibold rounded-md transition-colors ${authView === 'login' ? 'bg-purple-600 text-white shadow-md' : 'text-gray-500 hover:text-gray-700'}`}
                >
                  Log In
                </button>
                <button
                  onClick={() => { setAuthView('signup'); setAuthError(null); }}
                  className={`flex-1 py-2.5 text-sm font-semibold rounded-md transition-colors ${authView === 'signup' ? 'bg-purple-600 text-white shadow-md' : 'text-gray-500 hover:text-gray-700'}`}
                >
                  Sign Up
                </button>
              </div>
              ...
              {authView === 'login' ? (
                <form onSubmit={handleLogin} className="space-y-4">
                  ...
                  <button
                    ...
                    className="w-full py-3 bg-gradient-to-r from-purple-600 to-pink-500 text-white font-semibold rounded-lg hover:from-purple-700 hover:to-pink-600 transition-all disabled:opacity-50 flex items-center justify-center"
                  >
                    {authLoading ? <Loader className="h-5 w-5 animate-spin" /> : 'Log In'}
                  </button>
                </form>
              ) : (
                <form onSubmit={handleSignup} className="space-y-4">
                  ...
                  <button
                    ...
                    className="w-full py-3 bg-gradient-to-r from-purple-600 to-pink-500 text-white font-semibold rounded-lg hover:from-purple-700 hover:to-pink-600 transition-all disabled:opacity-50 flex items-center justify-center"
                  >
                    {authLoading ? <Loader className="h-5 w-5 animate-spin" /> : 'Create Account'}
                  </button>
                </form>
              )}
            </div>
      ...
      <main className={`flex-1 transition-all duration-300 ${sidebarOpen ? 'ml-64' : 'ml-0'} overflow-y-auto`}>
        ...
        <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between sticky top-0 z-10">
          ...
          <div className="flex items-center space-x-3">
            {authUser && (
              <span className="text-sm text-gray-600">Hi, <strong>{authUser.name}</strong></span>
            )}
            <button
